...
public class Rotate : BehaviourAI.IAction
{
    ...
    public void Execute()
    {
        ...
        {
            m_target.transform.Rotate(0,angle > 0 ? m_rotateSpeed : -m_rotateSpeed,0);
...
public class Attack : IAction
{
    ...
    public void Execute()
    {
        ...
        if (m_anim.GetCurrentAnimatorStateInfo(0).normalizedTime > 0.9f)
        {
            m_reset = true;
        }
    }
...
public class CheckDistance : BehaviourAI.IConditional
{
    ...
    public bool Check()
    {
        bool isClose = Vector3.Distance(m_targetObject.transform.position, (m_target == null ? CreatureAI.Target : m_target).transform.position) < m_distance;
        var log = isClose ? "近い" : "遠い";
        //Debug.Log($"{m_distance}より{log}");
        return isClose;
    }
}
